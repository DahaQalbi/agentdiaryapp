...
app.post("/addproperty", upload.array("files"), (req, res) => {
  ...
  connection.query(sql, [type, twon_house, building, unit, status], (err) => {
    ...
    if (status === "Rent") {
      const sql = "SELECT p_id FROM property WHERE p_id = (SELECT MAX(p_id) FROM property)"
      connection.query(sql, (err, result) => {
        if (err) throw err;
        else {
          const { p_id } = result[0];
          const sql2 = "INSERT INTO owner (p_id,owner_name,owner_nationality,owner_mobile_number) VALUES (?,?,?,?)";
          connection.query(sql2, [p_id, owner_name, owner_nationality, owner_mobile_number], (err, result) => {
            if (err) throw err;
            if (Array.isArray(checks) && checks.length > 0) {
              ...
              for (i = 0; i < clen; i++) {
                ...
                const sql4 = "INSERT INTO `rent_detail`(`p_id`, `start_date`, `end_date`, `total_checks`) VALUES (?,?,?,?)";
                connection.query(sql4, [p_id, start_date, end_date, check_val], (err) => {
                  if (err) throw err;
                }
                )
              }
              ...
              if (Array.isArray(files) && files.length > 0) {
                ...
                for (i = 0; i < len; i++) {
                  ...
                  const sql3 = "INSERT INTO `property_document`(`p_id`, `file`) VALUES (?,?)";
...
app.delete("/deleteproperty/:id", (req, res) => {
  const { id } = req.params;
  connection.query(
    "DELETE FROM property WHERE p_id = ? ",
    id,
    (err) => {
      if (err) throw err;
      connection.query("DELETE FROM owner WHERE p_id = ? ",
        id,
        (err) => {
          if (err) throw err;
          connection.query("DELETE FROM property_document WHERE p_id = ? ",
            id,
            (err) => {
              if (err) throw err;
              connection.query("DELETE FROM rent_detail WHERE p_id = ? ",
                id,
                (err, result) => {
                  if (err) {
...
app.get("/property/:id", (req, res) => {
  ...
  const sql = "SELECT * FROM property p JOIN owner own ON p.p_id = own.p_id LEFT JOIN rent_detail rd ON p.p_id = rd.p_id AND p.status = 'rent' WHERE p.p_id = ?";
...
app.put("/updates", (req, res) => {
  ...
  connection.query(sql,
    ...
    (err, result) => {
      if (err) throw err;
      const sql1 = "UPDATE `owner` SET  `owner_name`=?,`owner_nationality`=?,`owner_mobile_number`=? WHERE p_id =?";
        ...
        (err, result1) => {
          ...
          if (status === "Rent") {
            for (let i = 0; i < checks.length; i++) {
              const sql32 = "UPDATE `rent_detail` SET `start_date`=?,`end_date`=?,`total_checks`=? WHERE p_id = ?";
